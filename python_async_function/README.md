...
| `3-tasks.py` | Function `task_wait_random` that returns an `asyncio.Task` wrapping `wait_random(max_delay)`. |
| `4-tasks.py` | Coroutine `task_wait_n` that spawns `task_wait_random` n times and returns the delays in ascending order. |
